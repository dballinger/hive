...
import dballinger.Client.{Authentication, Path, Post}
import dballinger.models.{HttpFailure, UnparseableResponse}
import io.circe._
...
class Client(baseUrl: String) {
  ...
  private def http(path: Path, authentication: Authentication): HttpRequest = Http(s"$baseUrl$path").headers(
